...
.preview{border:1px solid var(--line);border-radius:12px;padding:20px;line-height:1.75}
...
.thumb-wrap{position:relative;border-radius:15px;overflow:hidden;background:#ddd}.thumb{width:100%;display:block}.thumb-badge{position:absolute;left:5%;bottom:8%;max-width:88%;background:rgba(20,20,20,.78);color:#fff;padding:13px 18px;border-radius:10px;font-weight:800;font-size:clamp(18px,3vw,34px)}
...
<label>본문 HTML 편집</label>
<textarea
  id="body_html_editor"
  name="body_html"
  style="min-height:430px"
  oninput="updateArticlePreview()"
>{{a.body_html}}</textarea>
...
<div style="margin-top:24px">
  ...
  <div>
    <label style="margin-top:0">📝 블로그 글 미리보기</label>
    <p class="small" style="margin:4px 0 0">
      실제 발행될 글의 모습을 먼저 확인하세요.
    </p>
  </div>
...
<div
  id="article_live_preview"
  class="preview article-live-preview"
>
  {{a.body_html|safe}}
</div>

<details class="html-editor-details">
  <summary>✏️ HTML 직접 편집하기</summary>

  <div class="html-editor-inner">
    <p class="notice">
      일반적으로는 위 미리보기만 확인하면 됩니다.
      HTML을 직접 수정할 때만 이 영역을 열어주세요.
    </p>
...
function updateArticlePreview() {
  const editor = document.getElementById("body_html_editor");
  const preview = document.getElementById("article_live_preview");
...
<section class="card"><h2>미리보기</h2><div class="preview"><h1>{{a.title}}</h1>{{a.body_html|safe}}{% if a.affiliate_enabled %}{{a.affiliate_html|safe}}{% endif %}</div></section>
  ...
  <div class="actions" style="justify-content:space-between;margin-top:0">
    <div>
      <h2>콘텐츠 패키지</h2>
      <p class="small">
        인스타그램, Threads, 릴스·쇼츠용 콘텐츠를 한눈에 확인하고 바로 복사하세요.
      </p>
    </div>

    <form
      method="post"
      action="{{url_for('generate_social_route',article_id=a.id)}}"
    >
      <button class="btn" type="submit">
        SNS 콘텐츠 다시 생성
      </button>
  ...
  <div class="instatoon-grid">
    ...
    <article class="idea-card">
      ...
      >{{a.instagram_caption}}</textarea>

      <button
        class="btn copy-btn"
        type="button"
        onclick="copyText('instagram_text',this)"
      >
        인스타 캡션 복사
      </button>
      {% else %}
      <p class="small">아직 생성된 인스타그램 캡션이 없습니다.</p>
      {% endif %}
    </article>
    ...
    <article class="idea-card">
      ...
      <textarea
        ...
        readonly
        style="min-height:260px"
      >{{a.threads_text}}</textarea>

      <button
        class="btn copy-btn"
        type="button"
        onclick="copyText('threads_text',this)"
      >
        Threads 글 복사
      </button>
      {% else %}
      <p class="small">아직 생성된 Threads 글이 없습니다.</p>
      {% endif %}
    </article>

  </div>

  <article class="idea-card" style="margin-top:14px">
    <span class="tag">Reels · Shorts</span>
    <h3>릴스·쇼츠 대본</h3>
    ...
    >{{a.shorts_script}}</textarea>

    <button
      class="btn copy-btn"
      type="button"
      onclick="copyText('shorts_text',this)"
    >
      릴스·쇼츠 대본 복사
    </button>
    {% else %}
    <p class="small">아직 생성된 릴스·쇼츠 대본이 없습니다.</p>
    {% endif %}
  </article>
</section>

{% if '🎨 인스타툰 8컷' in (a.notes or '') %}
{% set instatoon_text = ... %}
{% set instatoon_cuts = ... %}
...
<section class="card">
  <div
    class="actions"
    style="justify-content:space-between;align-items:flex-start;margin-top:0"
  >
    <div>

      <h2>🎨 인스타툰 8컷</h2>
      <p class="small">
        컷별 장면·대사·자막을 확인하고 각각 복사할 수 있어요.
      </p>
    </div>
    ...
    <button
      class="btn gray"
      type="button"
      onclick="copyText('instatoon_all_text', this)"
    >
      전체 복사
    </button>
  </div>
  ...
  <textarea
    id="instatoon_all_text"
    readonly
    style="position:absolute;left:-9999px"
  >{{instatoon_text}}</textarea>

  <div class="grid" style="margin-top:18px">
    {% for cut in instatoon_cuts %}
      {% if cut.strip() %}
      <article class="instatoon-card">
  <span class="instatoon-number">CUT {{loop.index}}</span>

  <div
    id="instatoon_cut_{{loop.index}}"
    class="instatoon-card-content"
  >{{cut.strip()}}</div>

        <button
          class="btn copy-btn"
          type="button"
          onclick="copyText('instatoon_cut_{{loop.index}}', this)"
        >
          이 컷 복사
        </button>
      </article>
