...
.stat-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin-top:16px}.stat{background:var(--soft);border:1px solid #e8e5ff;border-radius:14px;padding:16px}.stat strong{display:block;font-size:28px;margin-bottom:4px}.copy-box{position:relative}.copy-btn{margin-top:7px;background:#edf0f4;color:#202124}.calendar-item{padding:14px 0;border-bottom:1px solid var(--line)}.score{font-size:46px;font-weight:900}.check{padding:10px 0;border-bottom:1px solid var(--line)}.pass{color:var(--ok);font-weight:800}.fail{color:var(--bad);font-weight:800}.tags{display:flex;gap:7px;flex-wrap:wrap}.tag{background:var(--soft);color:#5046b8;padding:7px 10px;border-radius:99px}
@media(max-width:700px){.grid,.stat-grid{grid-template-columns:1fr}.wrap{margin-top:18px}.card{padding:18px}table thead{display:none}table tr,table td{display:block}table tr{padding:12px 0;border-bottom:1px solid var(--line)}table td{border:0;padding:4px 0}}
</style>
</head>
...
def home():
    ...
    return page("""
<div class="card">
<h1>MI Creator Hub <span class="status">V6.2</span></h1>
<p class="lead">키워드 하나로 글, SEO, 썸네일, Blogger 발행까지 한 흐름으로 만들어요.</p>
<div class="actions"><a class="btn gray" href="{{url_for('content_calendar')}}">콘텐츠 캘린더 보기</a></div>
<div class="stat-grid">
...
""", articles=articles, google_connected=google_connected,
       ...
       scheduled_count=scheduled_count, upcoming=upcoming,
